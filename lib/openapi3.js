...
function getResponseExamplesProtocol(data) {
    let content = '';
    let examples = [];
    let autoDone = {};
    for (let resp in data.operation.responses) {
        if (!resp.startsWith('x-')) {
            ...
            // copied from above - just a placeholder to hold status and meaning for population in the examples below
            let entry = {};
            entry.status = resp;
            entry.meaning = (resp === 'default' ? data.translations.responseDefault : data.translations.responseUnknown);
            var url = '';
            for (var s in common.statusCodes) {
    ...
    for (let example of examples) {
        // only show non error responses, and one of each type, i.e. 1 x 200 OK, 1x 201 Created, etc...
        if ((example.status && 300 <= example.status && example.status < 600) || existingStatuses.includes(example.status)) {
            continue
        }

        existingStatuses.push(example.status);

        if (common.doContentType(example.cta, 'json')) {
            if (example.status) {
                content += '```json &lcub;&lcub; title: \'' + example.status;
                if (example.meaning) {
                    content += ' - ' + example.meaning;
                }
                content += '\'&rcub;&rcub;\n';
            } else {
                content += '```json\n';
        ...
        if (common.doContentType(example.cta, 'yaml')) {
            if (example.status) {
                content += '```yaml &lcub;&lcub; title: \'' + example.status;
                if (example.meaning) {
                    content += ' - ' + example.meaning;
                }
                content += '\'&rcub;&rcub;\n';
        ...
        if (common.doContentType(example.cta, 'text')) {
            if (example.status) {
                content += '```text &lcub;&lcub; title: \'' + example.status;
                if (example.meaning) {
                    content += ' - ' + example.meaning;
                }
                content += '\'&rcub;&rcub;\n';
    ...
    // need to trim to remove empty whitespace - used in presence check in responses.def
    return content.trim();
}

function getSchemasForTag(data, tag) {
    let schemas = {};
    if (data.components && data.components.schemas) {
        for (const [s, schema] of Object.entries(data.components.schemas)) {
            if (schema.tags && schema.tags.length) {
                for (var t of schema.tags) {
                    if (t === tag) {
                        schemas[s] = schema;
                    }
                }
...
function convertInner(api, options) {
    return new Promise(function (resolve, reject) {
        ...
        data.utils.getResponseExamplesProtocol = getResponseExamplesProtocol;
        data.utils.getSchemasForTag = getSchemasForTag;
